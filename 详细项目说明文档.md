...
从代码结构和主流程定义来看，MASDiff 当前关注的问题可以概括为：

1. 在一个多智能体环境中先随机生成初始奖励 `R∈[0,5]`，并用 `R + A*` 完成初始基础仿真；
2. 通过环境仿真收集状态信息 `Tau` 和经验库；
3. 用扩散模型根据 `Tau` 生成奖励 `R`；
4. 直接用 `R + 纯 A*` 再次仿真，计算每个个体相对于目标 `Q` 的指标 `ρ`；
5. 将 `(Tau, R, ρ)` 作为种群个体，持续做扩散训练、精英选择、截断扩散变异和保优迭代。
...
当前配置中仍可保留 `dqn_module` 字段用于兼容旧配置，但主流程已不再调用 DQN 训练/推理。
...
#### 3. 初始化随机奖励矩阵
当前实现会先根据 `Q` 的最后一维推断 `num_road`，再随机生成形状为 `[N, num_road]`、取值范围为 `[0,5]` 的初始奖励矩阵 `R`，使环境在首次仿真时直接走 `R + A*` 分支。

#### 4.1 仿真并收集经验库与 Tau
调用：
- `Environment.simulate_collect(planner_input)`
...
调用：
- `Environment.simulate_evaluate(rewards)`
...
#### 5.3 对每个精英个体做奖励变异与再评估
对每个精英依次执行：

1. 使用截断扩散对原奖励做变异：
   - `generate_reward_truncated(tau, base_rewards, add_noise_steps, denoise_steps)`
2. 用变异后的 `R' + 纯 A*` 仿真，重新收集经验库和新的 `Tau`；
3. 根据新的 `Tau` 再生成奖励 `R`；
4. 用新的 `R + 纯 A*` 最终仿真并重新计算 `ρ`。
...
### 6. 导出最优奖励对应的规划路线（可选）
若环境实现了导出能力，则会直接使用最优个体的 `R + 纯 A*` 重新规划并输出新的 `rou` 文件。
...
### 输入
- `planner_input`：既可以是策略列表，也可以是奖励矩阵 `R`；
- 若传入奖励矩阵，则环境直接执行“R + 纯 A*”；
- 若传入策略列表且某个策略不可用或为 `None`，则该车辆退化为只使用 A*。
...
车辆首次出现时做路径规划：
- 有奖励矩阵 `R`：直接使用“R + 纯 A*”；
- 有 policy 且主流程显式传入策略时：沿路径逐边使用“DQN 选下一条边 + A* 启发式/回退”；
- 无奖励且无 policy：只使用按路段长度为代价的 A*。

因此当前主流程默认走的是“奖励引导的纯 A*”分支，DQN 仅作为保留实现存在。
