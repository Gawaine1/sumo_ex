...
def _evaluate_rewards_batch(
    *,
    ...
    # 支持串行执行器
    ray_executor = executor if isinstance(executor, RayExecutor) else None

    if ray_executor is not None:
        ...
        for rewards in rewards_batch:
            task = {
                ...
                "q": q,  # 直接传递 q 而不是 q_ref
                "environment_spec": environment_spec,
                "metric_spec": metric_spec,
            }
            result = ray_evaluate_reward_batch(task)
...
def run_masdiff(cfg: MasDiffConfig) -> Population:
    ...
    try:
        ...
        if cached_initial_population is not None:
            ...
            if ray_executor is not None:
                ...
                for i in range(1, M + 1):
                    ...
                    individual = ray_build_initial_individual(task)
        ...
        # 直接使用当前population作为最终种群，确保保存完整种群
        final_population = population
        
        # 保存最终种群
        if initial_population_path:
            final_population_path = Path(initial_population_path).parent / "final_population.pkl"
            step_save_final_population(final_population, population_path=str(final_population_path))
            print(f"最终种群已保存到：{final_population_path} (种群数量: {len(final_population)})")
