...
class SumoRylDiffusionModel(DiffusionModel):
    ...
    def random_init_rewards(self, population_size: int) -> list[torch.Tensor]:
        ...
        for _ in range(population_size):
            # 生成随机奖励，范围 [0, reward_scale]
            num_car = self._net_cfg["num_car"]
            # 从 tau 的形状推断 num_road
            # 这里假设 num_road 可以从某个地方获取，或者使用默认值
            # 暂时使用一个合理的默认值
            num_road = 114  # ChongQing-114 的道路数
            random_reward = torch.rand((num_car, num_road), dtype=torch.float32) * self.hparams.reward_scale
            rewards_list.append(random_reward)
        return rewards_list

    def generate_rewards_given_tau(self, tau: Any, n_samples: int = 1) -> list[torch.Tensor]:
        """给定条件 τ，用扩散模型采样 n_samples 个奖励张量。"""
        rewards_list = []
        for _ in range(n_samples):
            reward = self.generate_reward(tau)
            rewards_list.append(reward)
        return rewards_list

    def train_on_population(self, population: list[Any]) -> None:
        """用当前种群（包含 tau, rewards, rho）训练扩散模型（兼容旧接口）。"""
        """
        CPU-GPU 同步，强制显存连续。
        """
        tau_list = []
