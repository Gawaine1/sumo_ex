...
from src.pipeline.steps import (
    ...
    step_6_simulate_best_and_export_routes, step_record_best_simulation_data_if_improved,
)
...
def run_masdiff(cfg: MasDiffConfig) -> Population:
    ...
    try:
        ...
        if cached_initial_population is not None:
            ...
            # RayExecutor 特殊处理：
            # - runner 内部的闭包会捕获 q/模型等大对象，Ray 序列化时会触发 “remote function too large”
            # - 这里改为：用顶层 task 函数 + ray.put 大对象引用
            if (RayExecutor is not None) and isinstance(executor, RayExecutor):
                import ray  # type: ignore

                from src.parallel.ray_tasks import (
                    build_initial_individual as ray_build_initial_individual,
                    extract_diffusion_state,
                )

                q_ref = ray.put(q)
                diffusion_state_ref = ray.put(extract_diffusion_state(diffusion_model))
                    ...
                    tasks.append(
                        {
                            "i": i,
                            "N": N,
                            # 给每个任务不同 seed，避免 worker 内完全一致（同时仍可复现）
                ...
                def build_initial_individual(i: int) -> Individual:
                    ...
                    # 关键：不保存 policies（巨大且后续未使用，会导致内存/序列化爆炸）
        ...
        for k in range(1, K + 1):
            ...
            if (RayExecutor is not None) and isinstance(executor, RayExecutor):
                ...
                from src.parallel.ray_tasks import mutate_one as ray_mutate_one, extract_diffusion_state

                # 与初始种群一致：把 Q 与“本迭代训练后的 diffusion 参数”放进 object store
                q_ref = ray.put(q)
                diffusion_state_ref = ray.put(extract_diffusion_state(diffusion_model))

                tasks = []
                for idx, elite in enumerate(elites):
                    # 再保险：避免把 policies（巨大）传进 Ray
                    elite.policies = []
                    tasks.append(
                        {
                            "elite": elite,
                            "iteration_k": k,
                            "seed": int(cfg.seed) + int(k) * 100000 + int(idx),
                            "q_ref": q_ref,
                            "diffusion_state_ref": diffusion_state_ref,
                            "truncated_diffusion": {
                                "add_noise_steps": int(cfg.truncated_diffusion.add_noise_steps),
                ...
                mutants = executor.map(ray_mutate_one, tasks)
            else:
