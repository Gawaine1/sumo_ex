        ...
        if self.cache_path and (not self.force_recompute):
            ...
            if p.exists() and p.stat().st_size > 0:
                try:
                    ...
                    # PyTorch 2.6+ 默认 weights_only=True，旧缓存可能因此加载失败。
                    ...
                    if maybe_weights_only_issue:
                        try:
                            q = torch.load(str(p), map_location="cpu", weights_only=False)
                            print(
                                f"警告: 默认安全模式加载失败，已使用 weights_only=False 兼容加载缓存 Q。"
                                f"仅应对可信来源文件使用此回退。路径: {p}"
                            )
                            print(f"加载q:{q}")
                            return q
                        except Exception as e2:
                            print(f"警告: 兼容模式加载缓存 Q 也失败，路径: {p}，将重算。错误信息: {e2}")
                    else:
                        print(f"警告: 加载缓存的 Q 失败，路径: {p}，将重算。错误信息: {e}")
                    # 缓存不可用时回退到重算
                    pass
